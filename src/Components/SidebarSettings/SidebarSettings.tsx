...
function SidebarSettings() {
  ...
  const setRepoOwner = useStoreActions((actions: StoreActions) => actions.setRepoOwner)

  const repoName = useStoreState((state: StoreState) => state.repoName);
  const setRepoName = useStoreActions((actions: StoreActions) => actions.setRepoName)

  const githubToken = useStoreState((state: StoreState) => state.githubToken);
  const setGithubToken = useStoreActions((actions: StoreActions) => actions.setGithubToken)

  const fetchIssues = async (githubToken: string, repoOwner: string, repoName: string) => {
      ...
      <div className='SidebarSettings_content'>
        <div>
          <div className='SidebarSettings_mainHeading'>
            ...
            <Button view='clear' iconLeft={IconClose} onlyIcon size='s' onClick={closeSidebar} />
          </div>
          <Text view='secondary' size='l' className='SidebarSettings_introParagraph'>
            IssueDeck — утилита для просмотра issues в открытых репозиториях GitHub.
            По умолчанию подключен репозиторий дизайн-системы
            <Text view='link' size='l' as='a' href=''> Consta</Text>.
          </Text>
          <Informer label="Для работы IssueDeck вам понадобится токен GitHub" view="filled" status="system" className='SidebarSettings_informer' />
          <InputWithLabel
        ...
        <Button
          ...
          onClick={() => {
            if (githubToken && repoOwner && repoName) {
              fetchIssues(githubToken, repoOwner, repoName).then(data => console.log(data))
            } else {
